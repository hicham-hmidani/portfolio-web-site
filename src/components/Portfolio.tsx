...
export default function Portfolio() {
  ...
  const [hero, setHero] = useState({
    ...
    bio: "A diligent and detail-oriented Junior Data Analyst/Scientist, AI Prompt Engineer, and Database Administrator holding a Master's degree in Computer Science and Complex Systems Engineering. I have a solid foundation in statistical analysis, machine learning, prompt engineering, and domain name investing.",
    available: true,
    rate: "$10/hr"
  });
  ...
  useEffect(() => {
    const unsubHero = onSnapshot(doc(db, 'portfolio_content', 'hero'), (doc) => {
      if (doc.exists()) setHero(prev => ({ ...prev, ...doc.data() }));
    });
    const unsubContact = onSnapshot(doc(db, 'portfolio_content', 'contact'), (doc) => {
      if (doc.exists()) setContact(doc.data() as any);
    });
    const unsubUpwork = onSnapshot(doc(db, 'marketplace_items', 'upwork_config'), (doc) => {
      if (doc.exists()) {
        const data = doc.data();
        if (data.hourlyRate) {
          setHero(prev => ({ ...prev, rate: `$${data.hourlyRate}/hr` }));
        }
      }
    });
    return () => {
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-teal-500/30 overflow-x-hidden relative">
      ...
      <main id="about" className="relative z-10 container mx-auto px-6 pt-32 pb-32 md:pt-40 md:pb-40 flex flex-col lg:flex-row items-center gap-16">
        ...
        <div className="flex-1 flex flex-col items-start">
          ...
          >
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white border border-slate-200 text-green-600 text-xs font-medium shadow-sm backdrop-blur-sm">
              <span className="relative flex h-2 w-2">
                <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${hero.available ? 'bg-green-400' : 'bg-red-400'} opacity-75`}></span>
                <span className={`relative inline-flex rounded-full h-2 w-2 ${hero.available ? 'bg-green-500' : 'bg-red-500'}`}></span>
              </span>
              {hero.available ? 'Available for Freelance' : 'Currently Unavailable'}
            </div>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white border border-slate-200 text-teal-600 text-xs font-medium shadow-sm backdrop-blur-sm">
              <span className="font-bold">{hero.rate}</span> on Upwork
            </div>
          </motion.div>
          ...
          >
            Hi, I'm <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-600 to-yellow-500">{hero.name}</span>
            <br />
